...
void FileIo::WriteValueToFile(const std::string &file_path, const uint32_t data, const bool append) {
    ...
    // Close the file
    file.close();
    utils::Logger::TraceLog(LOCATION, "Value has been written to the file (" + file_path + this->ext_ + ")", this->debug_);
}
...
void FileIo::WriteVectorToFile(const std::string &file_path, const std::vector<uint32_t> &data, const bool append) {
    ...
    // Close the file
    file.close();
    utils::Logger::TraceLog(LOCATION, "Numbers have been written to the file (" + file_path + this->ext_ + ")", this->debug_);
}
...
void FileIo::WriteStringToFile(const std::string &file_path, const std::string &data, const bool append) {
    ...
    // Close the file
    file.close();
    utils::Logger::TraceLog(LOCATION, "String have been written to the file (" + file_path + this->ext_ + ")", this->debug_);
}
...
void FileIo::WriteStringVectorToFile(const std::string &file_path, const std::vector<std::string> &data, const bool append) {
    ...
    // Close the file
    file.close();
    utils::Logger::TraceLog(LOCATION, "Strings have been written to the file (" + file_path + this->ext_ + ")", this->debug_);
}
...
void FileIo::ReadValueFromFile(const std::string &file_path, uint32_t &data) {
    ...
    // Close the file
    file.close();
    utils::Logger::TraceLog(LOCATION, "Value read from file (" + file_path + this->ext_ + "): " + std::to_string(data), this->debug_);
}
...
bool FileIo::OpenFile(std::ofstream &file, const std::string &file_path, const std::string &location, const bool append) {
    ...
    if (!file.is_open()) {
        utils::Logger::ErrorLog(location, "Failed to open file for writing. (" + file_path + this->ext_ + ")");
        return false;
    }
    return true;
}
...
bool FileIo::OpenFile(std::ifstream &file, const std::string &file_path, const std::string &location) {
    ...
    if (!file.is_open()) {
        utils::Logger::ErrorLog(location, "Failed to open file for reading. (" + file_path + this->ext_ + ")");
        return false;
    }
    return true;
}
...
uint32_t FileIo::ReadNumCountFromFile(std::ifstream &file, const std::string &location) {
    ...
    if (std::getline(file, line)) {
        ...
        if (!(iss >> count)) {
            utils::Logger::ErrorLog(location, "Invalid file format: Unable to read the number of elements");
            return 0U;
        }
    }
    return count;
